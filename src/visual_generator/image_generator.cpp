 ...
 */
 ...
 #include "visual_generator/image_generator_impl.hpp"
 #include <string>
 #include <stdexcept>
            ...
            for(int j = 0; j < columns; ++j) {
                ...
                switch(population[i][j].getIndividualState()) {
                    ...
                    case Simulator::IndividualState::Healthy:
                        imageBuffer[bufferIndex + 0] = 0;
                        imageBuffer[bufferIndex + 1] = 255;
                        imageBuffer[bufferIndex + 2] = 0;
                    break;

                    case Simulator::IndividualState::Isolated:
                        imageBuffer[bufferIndex + 0] = 205;
                        imageBuffer[bufferIndex + 1] = 127;
                        imageBuffer[bufferIndex + 2] = 50;
                    break;

                    case Simulator::IndividualState::Sick:
                        imageBuffer[bufferIndex + 0] = 255;
                        imageBuffer[bufferIndex + 1] = 255;
                        imageBuffer[bufferIndex + 2] = 0;
                    break;
